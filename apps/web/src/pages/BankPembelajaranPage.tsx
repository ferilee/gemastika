...
export function BankPembelajaranPage() {
  ...
  const canReview = hasRole(user, "admin") || hasRole(user, "pengurus");
  ...
  const [reportTarget, setReportTarget] = useState<LearningResource | null>(null);
  ...
  async function submitReport() {
    ...
    setReporting(true);
    try {
      await api(`/api/learning-resources/${reportTarget.id}/reports`, { method: "POST", body: JSON.stringify({ reason: reportReason.trim(), detail: reportDetail.trim() }) });
      setReportTarget(null); setReportReason(""); setReportDetail("");
      alert("Laporan telah dikirim untuk ditinjau pengurus.");
    } catch (error) {
  ...
  async function restoreVersion(version: LearningResourceVersion) { if (!selected) return; try { const updated = await api<LearningResource>(`/api/learning-resources/${selected.id}/restore/${version.id}`, { method: "POST" }); setSelected(updated); setResources((items) => items.map((item) => item.id === updated.id ? updated : item)); alert("Versi dipulihkan dan dikirim untuk review ulang."); } catch (error) { alert(error instanceof Error ? error.message : "Gagal memulihkan versi."); } }
          ...
          <Card key={item.id} className="flex flex-col overflow-hidden transition hover:border-mgmp-primary/40">
            <button type="button" className="flex flex-1 flex-col text-left" onClick={() => navigate(permalink(item))}>
              ...
              <CardContent className="flex flex-1 flex-col p-5">
                ...
                <div className="mt-3 line-clamp-2 text-lg font-extrabold text-slate-800 dark:text-white">{item.title}</div>{item.publishStatus === "rejected" && item.reviewNote ? <div className="mt-2 line-clamp-2 text-xs font-semibold text-rose-700 dark:text-rose-300">Catatan reviewer: {item.reviewNote}</div> : null}
            ...
            <div className="flex justify-between gap-2 px-5 pb-5">{collections.length ? <Button size="sm" variant="secondary" onClick={() => { const collection = collections.find((row) => row.id === Number(activeCollectionId)) || collections[0]; void toggleCollection(collection.id, item); }}><Heart className="h-4 w-4" /> Simpan ke Koleksi</Button> : <span />}<div className="flex gap-2">{user ? <Button size="icon" variant="secondary" title="Laporkan materi" aria-label="Laporkan materi" onClick={() => { setReportTarget(item); setReportReason(""); setReportDetail(""); }}><Flag className="h-4 w-4" /></Button> : null}{canManage ? <Button size="sm" className="bg-rose-600 text-white hover:bg-rose-500 dark:bg-rose-500 dark:hover:bg-rose-400" disabled={deleting === item.id} onClick={() => setDeleteTarget(item)}><Trash2 className="h-4 w-4" /><span className="hidden sm:inline">Hapus</span></Button> : null}</div></div>
          </Card>
        ))}
      </div>
